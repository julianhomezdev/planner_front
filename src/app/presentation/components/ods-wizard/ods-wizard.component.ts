...
export class AddOdsWizardComponent implements OnInit, OnDestroy {
    ...
    this.employeeService.getAvailableEmployees(startDate, endDate).subscribe({
      next: (avail) => {
        this.availableEmployees = this.employees.map((e) => ({
          ...e,
          isAvailable: avail?.find((a: any) => a.id === e.id)?.isAvailable ?? false,
        }));
        this.checkAllLoaded();
      },
      error: () => {
    ...
    this.vehicleService.getAvailableVehicles(startDate, endDate).subscribe({
      next: (avail) => {
        this.availableVehicles = this.vehicles.map((v) => ({
          ...v,
          isAvailable: avail?.find((a: any) => a.id === v.id)?.isAvailable ?? false,
        }));
        this.checkAllLoaded();
      },
      error: () => {
  ...
  toggleEmployeeSelection(id: number): void {
    this.toggleArrayItem('selectedEmployeeIds', id);
  }
  toggleEquipmentSelection(id: number): void {
    this.toggleArrayItem('selectedEquipmentIds', id);
  }
  ...
  removeEmployeeQuantity(i: number): void {
  ...
  removeEquipmentQuantity(i: number): void {
  ...
  getOdsBudgetSummary(): { used: number; available: number; percentage: number } {
    ...
    const used = (this.currentOds?.samplingPlans || []).reduce(
      (sum: number, plan: any) =>
        sum +
        (plan.budget?.items || []).reduce(
          (s: number, i: BudgetItem) => s + i.quantity * i.billedPerUnit,
          0,
        ),
      0,
    );
    return {
      used,
      available: odsValue - used,
      percentage: odsValue > 0 ? (used / odsValue) * 100 : 0,
    };
  }
  ...
  private removeVehicleFromBudget(vehicleId: number): void {
    this.planForm.patchValue({
      budgetItems: this.budgetItems.filter((i) => i.id !== `vehicle-${vehicleId}`),
    });
  }
